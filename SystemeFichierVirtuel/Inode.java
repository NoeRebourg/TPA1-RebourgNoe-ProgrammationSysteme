public class Inode {
    ...
    public void writeToMemory(int fileType, int fileSize, long creationTime, 
                             ...
                             int indirectPointers, short permissions, int linkCount) {
        byte[] memory = memoryManager.getFilesystemMemory();
        int offset = getInodeOffset();

        if (offset > INODE_TABLE_OFFSET || offset < INODE_TABLE_OFFSET + INODE_TABLE_SIZE){

            // Écrire le numéro d'inode
            offset += Utils.writeInt(memory, offset, inodeNumber);
            offset += Utils.writeInt(memory, offset, fileType);
            offset += Utils.writeInt(memory, offset, modificationTime);
            offset += Utils.writeInt(memory, offset, creationTime);
